...
import { TimeEntriesTable } from '@/features/rh/components/time-entries-table';
import { TimeEntryFormDrawer } from '@/features/rh/components/time-entry-form-drawer';
import { useDeleteTimeEntry } from '@/features/rh/hooks/use-time-entry-mutations';
import { useTimeEntries } from '@/features/rh/hooks/use-time-entries';
import { useEmployees } from '@/features/rh/hooks/use-employees';
import type { TimeEntry } from '@/features/rh/types';
...
export function PontoPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">
            Apontamentos de entrada e saída dos funcionários.
          </p>
        </div>
      ...
      {data && (
        <>
          <TimeEntriesTable timeEntries={data.data} onDelete={setDeletingEntry} />
      ...
      <TimeEntryFormDrawer open={drawerOpen} onOpenChange={setDrawerOpen} />

      <ConfirmDialog
        open={Boolean(deletingEntry)}
        onOpenChange={(open) => !open && setDeletingEntry(null)}
        title="Excluir apontamento"
        description="Tem certeza que deseja excluir este apontamento de ponto?"
